...
public class MemberServiceImpl implements MemberService {
	...
	public Map<String, Object> idCheck(String id) {
		...
		MemberRepository memberRepository = sqlSession.getMapper(MemberRepository.class);
		map.put("result", memberRepository.duplicateIdCheck(id));
		return map;
	}
	...
	public Map<String, Object> memberInfo(HttpServletRequest request) {
		...
		try {
			...
			if (selectResult == null)
				throw new PersistenceException("일치하는 회원정보가 없습니다.");
	...
	public Map<String, Object> modifyMember(Member m, HttpServletRequest request) {
		...
		try {
			...
			if (birthday.length() != 8)
				throw new NullPointerException("생일 정보가 없습니다");
			if (name.isEmpty())
				throw new NullPointerException("입력된 이름이 없습니다");
			if (phone.isEmpty())
				throw new NullPointerException("입력된 핸드폰 번호가 없습니다");
			if (phone.length() != 11)
				throw new NullPointerException("올바른 형식이 아닙니다.");
